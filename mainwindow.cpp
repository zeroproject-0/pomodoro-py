...
MainWindowSmall::MainWindowSmall(QWidget* parent)
	: QMainWindow(parent)
{
	ui.setupUi(this);
	...
	timer = new QTimer();
	QObject::connect(timer, &QTimer::timeout, this, &MainWindowSmall::updateTimer);
	...
	start_action = new QAction("Start");
	QObject::connect(start_action, &QAction::triggered, this, &MainWindowSmall::start);
	context_menu->addAction(start_action);

	stop_action = new QAction("Stop");
	QObject::connect(stop_action, &QAction::triggered, this, &MainWindowSmall::stop);
	context_menu->addAction(stop_action);

	context_menu->addSeparator();

	QAction* settings_action = new QAction("Settings");
	QObject::connect(settings_action, &QAction::triggered, this, &MainWindowSmall::tray_open_settings);
	context_menu->addAction(settings_action);

	QAction* always_top_action = new QAction("Always on top");
	QObject::connect(always_top_action, &QAction::triggered, this, &MainWindowSmall::tray_always_top);
	always_top_action->setCheckable(true);
	always_top_action->setChecked(isAlwaysTop);
	context_menu->addAction(always_top_action);
	...
	QAction* minimize_action = new QAction("Minimize");
	QObject::connect(minimize_action, &QAction::triggered, this, &MainWindowSmall::tray_minimize);
	context_menu->addAction(minimize_action);

	QAction* exit_action = new QAction("Exit");
	QObject::connect(exit_action, &QAction::triggered, this, &MainWindowSmall::tray_exit);
	context_menu->addAction(exit_action);
...
MainWindowSmall::~MainWindowSmall()
{
	delete timer;
	delete progressbar;
	delete context_menu;
...
void MainWindowSmall::displayCounterTime(int total_time)
{
	int time = total_time - current_time;
	int minutes = time / 60;
	int seconds = time % 60;
...
void MainWindowSmall::start()
{
	if (timer->isActive()) {
		...
		timer->start();
		start_action->setText("Pause");
	}
}

QString MainWindowSmall::parseDisplayTime(int time) {
	QString stime = QString::number(time);
	return time < 10 ? "0" + stime : stime;
}

void MainWindowSmall::load_settings()
{
	pomo_time = Settings::load_settings("pomo_time", 1500, "SettingsWindow").value<int>();
	rest_time = Settings::load_settings("rest_time", 300, "SettingsWindow").value<int>();
	isAlwaysTop = Settings::load_settings("always_top", false, GROUP).value<bool>();
...
void MainWindowSmall::reset_timer(int total_time)
{
	timer->stop();
	current_time = 0;
	start_action->setText("Start");
...
void MainWindowSmall::open_settings()
{
	if (timer->isActive()) {
		timer->stop();
		start_action->setText("Start");
	...
	QObject::connect(settingsWindow, &QDialog::accepted, this, &MainWindowSmall::updateSettings);
}

void MainWindowSmall::toggle_always_top()
{
	isAlwaysTop = !isAlwaysTop;
	Settings::save_settings("always_top", isAlwaysTop, GROUP);
...
void MainWindowSmall::paintEvent(QPaintEvent* e)
{
	progressbar->paintProgress();
	e->accept();
}

void MainWindowSmall::mousePressEvent(QMouseEvent* e)
{
	QPointF pos = e->globalPos();
	old_position = new QPoint((int)pos.x(), (int)pos.y());
}

void MainWindowSmall::mouseMoveEvent(QMouseEvent* e)
{
	QPoint* delta = new QPoint(e->globalPos() - *old_position);
	this->move(this->x() + delta->x(), this->y() + delta->y());
	*old_position = e->globalPos();
...
void MainWindowSmall::contextMenuEvent(QContextMenuEvent* e)
{
	(context_menu->exec(e->globalPos()));
}

void MainWindowSmall::on_btnExit_clicked()
{
	this->close();
}

void MainWindowSmall::on_btnMinimize_clicked()
{
	this->showMinimized();
}

void MainWindowSmall::on_btnOpenSettings_clicked()
{
	open_settings();
}

void MainWindowSmall::on_btnIcon_clicked()
{
	QDesktopServices::openUrl(QUrl(QApplication::organizationDomain()));
}

void MainWindowSmall::tray_open_settings()
{
	open_settings();
}

void MainWindowSmall::tray_always_top()
{
	toggle_always_top();
}

void MainWindowSmall::tray_minimize()
{
	this->showMinimized();
}

void MainWindowSmall::tray_exit()
{
	this->close();
}

void MainWindowSmall::updateTimer()
{
	int time = isPomoTime ? pomo_time : rest_time;
	timer->setInterval(1000);
	progressbar->set_value(current_time * progressbar->max_value / time);
...
void MainWindowSmall::updateSettings()
{
	load_settings();
	reset_timer(pomo_time);
}

void MainWindowSmall::stop()
{
	isPomoTime = true;
	this->reset_timer(pomo_time);
}
